...
const {
	...
	redeemInvite,
	setAccess
} = await import('$lib/server/bot/access');
...
describe('останній власник', () => {
	const owner = { ...user, role: 'ADMIN' as const };

	it('менеджера не рахуємо взагалі — база навіть не чіпається', async () => {
		await expect(isLastAdmin({ ...user, role: 'MANAGER' } as never)).resolves.toBe(false);
		expect(db.botUser.count).not.toHaveBeenCalled();
	});

	it('єдиний адмін — останній', async () => {
		db.botUser.count.mockResolvedValue(0);

		await expect(isLastAdmin(owner as never)).resolves.toBe(true);
		// Себе з підрахунку викидаємо, інакше останнім не був би ніхто.
		expect(db.botUser.count.mock.calls[0][0].where).toMatchObject({
			role: 'ADMIN',
			isActive: true,
			leftAt: null,
			id: { not: 'u1' }
	...
	it('коли є другий адмін, виходу ніщо не заважає', async () => {
		db.botUser.count.mockResolvedValue(1);

		await expect(isLastAdmin(owner as never)).resolves.toBe(false);
	});
});
